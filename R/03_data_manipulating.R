...
# The files are all in .csv format, separated by commas. So let's use the
#`read.csv()` function. We could also use `read.table()` with the arguments
#`sep = ","` and `header = TRUE` So let's read the five sets of data.
# A very useful function for reading data is the `list.files()` from the
#**base** package. This function lists files in a directory, based on a pattern.
...
#its important to change the data class for factor because, as the
#Site IDs are number, if it remain numeric we can accidentally do
...
envir_coord <- merge(x = envir,
                     y = coord,
                     by = "Sites")
# Here we are binding the sites composition present on "envir" with the
#coordenates for those present in "coord" by those commun column "sites"
#Its a good idea to turn this comun column in a "factor", to each
#observation to work as level
...
comm_df <- tidyr::pivot_longer(comm,
                               ...
                               names_to = "TaxCode",
                               values_to = "Abundance")
# The structure with package_name::function_name its used to specify the package
# ypu want this function to came from Thats important because sometimes different
#packages functions with the same name
...
write_excel_csv(x = comm_total,
                file = "data/processed/03_Pavoine2_full_table.csv")
# Extra: filter ----------------------------------------------------------------

#Selecting all information of species one and saving it in
#sp1 object
